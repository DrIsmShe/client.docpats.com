...
export default async function handler(request, context) {
  ...
  if (url.pathname === "/") {
    ...
    <script type="application/ld+json" data-seo="edge">${JSON.stringify({
      ...
      /* Год регистрации домена (10.03.2023, публичный WHOIS). Дата
         намеренно проверяемая: её можно сверить с реестром.
         legalName и address здесь нет — юрлица у проекта нет, страна
         регистрации не выбрана. Поля необязательные, а выдуманное
         название компании проверяется по реестру за минуту: не
         найдётся — доверия станет меньше, чем при пустом поле. */
      foundingDate: "2023",
  ...
  try {
    ...
    if (articleMatch) {
      ...
      /* В ИНДЕКС ИДЁТ НЕ ВЕСЬ ДАЙДЖЕСТ.
       *
       * Права на текст вопроса не вызывают — изложение наше. Вопрос в
       * качестве: правило Google о массовом производстве страниц смотрит
       * не на авторство, а на то, не сделаны ли тысячи страниц ради
       * выдачи. Шесть с половиной тысяч материалов на пяти языках — это
       * 33 тысячи страниц по три-четыре предложения, и такая форма
       * подпадает под правило независимо от того, кто их написал.
       * Санкция накладывается на домен, то есть заодно на витрины клиник
       * и профили врачей.
       *
       * Поэтому индексируется крепкая часть: материалы, у которых
       * распознан дизайн исследования (метаанализ, РКИ, когорта). Для
       * остальных — noindex, follow: страница работает, ссылки с неё
       * живые, поисковику она не предлагается.
       *
       * Тот же отбор в карте сайта (common/sitemap/services/
       * sitemap.service.js, УРОВНИ_В_КАРТЕ) — они обязаны совпадать.
       */
      noIndex = !["high", "moderate"].includes(item.evidenceLevel);
