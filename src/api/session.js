...
/**
 * Погасить пропуск студии DP-Videra.
 *
 * У студии своя печенька на 12 часов, выданная этим же доменом. Выход из
 * DocPats её не трогал: человек «выходил», а студия ещё полсуток
 * показывала его фильмы и давала снимать новые. Гасить её с сервера
 * нельзя — API отвечает с другого домена, а печенька принадлежит этому.
 *
 * Ошибку глотаем: студия могла быть не развёрнута рядом, и выход из
 * DocPats не должен от этого срываться.
 */
function забытьСтудию() {
  try {
    fetch("/dp-videra/exit", {
      method: "GET",
      credentials: "include",
      // Ответ нам не нужен — важен только Set-Cookie, который его гасит.
      redirect: "manual",
      cache: "no-store",
    }).catch(() => {});
  } catch {
    /* пусто: выход важнее уборки */
  }
}
