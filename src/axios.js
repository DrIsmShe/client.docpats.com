...
// И на ГЛОБАЛЬНЫЙ axios тоже.
//
// Через настроенные экземпляры ходят 44 файла, а голым axios — больше
// пятисот: язык доезжал до сервера меньше чем в десятой части запросов.
// Пока это так, любые переводы серверных сообщений бессмысленны — сервер
// не знает, на каком языке отвечать.
//
// Перехватчик на общем экземпляре покрывает все вызовы разом, а условие
// «не перебивать уже заданный заголовок» сохраняет прежнее поведение
// страниц, которые выбирают язык контента сами.
attachLanguage(axios);
