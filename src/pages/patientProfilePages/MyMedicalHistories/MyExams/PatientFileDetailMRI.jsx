// client/src/pages/patient/PatientFileDetailMRI.jsx
...
/** Строгое распознавание шифра "iv:data" (hex:hex), чтобы не путать с датой */
function looksEncryptedIvData(s) {
  if (typeof s !== "string") return false;
  return /^[0-9a-fA-F]{32}:[0-9a-fA-F]+$/.test(s);
}

/** Универсальная нормализация разных представлений даты в Date */
function normalizeDateInput(v) {
  if (!v && v !== 0) return null;

  // объект {day, month, year}
  ...
  // число или строковый таймстамп
  if (typeof v === "number" || /^\d+$/.test(String(v))) {
    const n = Number(v);
    const ms = n < 1e12 ? n * 1000 : n; // секунды -> мс
    const d = new Date(ms);
...
/** Достаём дату рождения из разных ключей/вложенностей пациента */
...
/** Возвращаем "ДД месяц ГГГГ (N лет)"; если DOB нет — пробуем просто возраст */
...
function decryptLike(v) {
  if (!v) return "";
  const s = String(v);
  if (s.includes(":")) return ""; // фронт не расшифровывает реальный шифр
  return s;
}
...
/* ===================== Основной компонент ===================== */
export default function PatientFileDetailMRI() {
  ...
  const handleDownloadPDF = async () => {
    ...
    if (imgH <= pageH - margin * 2) {
      ...
      pdf.save(`mri-summary-${item?.id || item?._id || "report"}.pdf`);
      return;
    }
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      setError("");
      try {
        const url = `${BASE_URL}/patient-profile/get-my-mri-file-details/files/${encodeURIComponent(
          id,
        )}`;
        const res = await axios.get(url, { withCredentials: true });
        if (!cancelled) {
          if (res?.data?.ok && (res?.data?.item || res?.data?.data)) {
            setItem(res.data.item || res.data.data);
  ...
  // готовая строка "ДД месяц ГГГГ (N лет)" или "—"
            ...
            <section style={card}>
              ...
              >
                <div>
                  ...
                  />
                  {/* В схеме может быть radiationDose, но для МРТ обычно неактуально */}
