// client/src/pages/patient/PatientFileDetailCoronographyScan.jsx
...
/* ===================== Главный компонент ===================== */
export default function PatientFileDetailCoronographyScan() {
  const { id } = useParams();
  ...
  const handleDownloadPDF = async () => {
    ...
    if (imgH <= pageH - margin * 2) {
      ...
      pdf.save(`coronography-summary-${item?._id || "report"}.pdf`);
      return;
    }
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      setError("");
      try {
        // Маршрут контроллера Coronography:
        const url = `${BASE_URL}/patient-profile/get-my-coronography-file-details/files/${encodeURIComponent(
          id
        )}`;
    ...
    >
      ...
      {!loading && !error && item && (
        <>
          ...
          <section style={card}>
            <h3 style={h3}>Кардиопараметры / мониторинг</h3>
            <KV
