// client/src/pages/patient/PatientFileDetailPETSCAN.jsx
...
/* ===================== Основной компонент ===================== */
export default function PatientFileDetailPETSCAN() {
  const { id } = useParams();
  ...
  const handleDownloadPDF = async () => {
    ...
    if (imgH <= pageH - margin * 2) {
      ...
      pdf.save(`pet-summary-${item?._id || "report"}.pdf`);
      return;
    }
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      setError("");
      try {
        const url = `${BASE_URL}/patient-profile/get-my-pet-scan-file-details/files/${encodeURIComponent(
          id
        )}`;
    ...
    >
      ...
      >
        ...
        <h1 style={{ margin: 0, fontSize: 22, fontWeight: 700 }}>
          Детали ПЭТ/ПЭТ-КТ исследования
        </h1>
      </div>
