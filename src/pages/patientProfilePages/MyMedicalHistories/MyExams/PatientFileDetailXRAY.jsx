// client/src/pages/patient/PatientFileDetailXRAY.jsx
...
/* ===================== Основной компонент ===================== */
export default function PatientFileDetailXRAY() {
  const { id } = useParams();
  ...
  const handleDownloadPDF = async () => {
    ...
    if (imgH <= pageH - margin * 2) {
      ...
      pdf.save(`xray-summary-${item?._id || "report"}.pdf`);
      return;
    }
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      setError("");
      try {
        const url = `${BASE_URL}/patient-profile/get-my-xray-file-details/files/${encodeURIComponent(
          id
        )}`;
