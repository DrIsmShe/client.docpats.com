...
export default function PatientFileDetailCapsuleEndoscopy() {
  ...
  const handleDownloadPDF = async () => {
    ...
    if (imgH <= pageH - margin * 2) {
      ...
      pdf.save(`capsule-endoscopy-summary-${item?._id || "report"}.pdf`);
      return;
    }
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        ...
        const url1 = `${BASE_URL}/patient-profile/get-my-capsule-endoscopy-file-details/files/${encodeURIComponent(
          id,
        )}`;
        let res;
        try {
          ...
          // Резерв: если бекенд назван по-другому (например, без "file")
          const url2 = `${BASE_URL}/patient-profile/get-my-capsule-endoscopy-file-details/files/${encodeURIComponent(
            id,
          )}`;
                  ...
                  {item.images.map((src, i) => {
                    ...
                    return (
                      ...
                      >
                        <img
                          src={href}
                          alt={`Capsule endoscopy image ${i + 1}`}
                          style={thumbImg}
                          loading="lazy"
                        />
                      </a>
