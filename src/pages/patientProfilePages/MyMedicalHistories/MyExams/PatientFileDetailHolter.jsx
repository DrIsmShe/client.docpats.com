// client/src/pages/patient/PatientFileDetailHolter.jsx
...
/* ===================== Основной компонент ===================== */
export default function PatientFileDetailHolter() {
  const { id } = useParams();
  ...
  const handleDownloadPDF = async () => {
    ...
    if (imgH <= pageH - margin * 2) {
      ...
      pdf.save(`holter-summary-${item?._id || "report"}.pdf`);
      return;
    }
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      setError("");
      try {
        const url = `${BASE_URL}/patient-profile/get-my-holter-file-details/files/${encodeURIComponent(
          id
        )}`;
    ...
    >
      ...
      {!loading && !error && item && (
        <>
          ...
          {/* HOLTER-специфика */}
          <section style={card}>
            <h3 style={h3}>Параметры холтер-мониторирования</h3>
            <KV
              label="Длительность мониторинга (часы)"
              value={item.monitoringDuration ?? "—"}
            />
            <KV label="Макс. ЧСС" value={item.maxHeartRate ?? "—"} />
            <KV label="Мин. ЧСС" value={item.minHeartRate ?? "—"} />
            <KV
              label="Эпизоды аритмий"
              value={safeJoin(item.arrhythmiaEpisodes)}
            />

            {/* Если у вас есть эти поля в модели — тоже покажем */}
            <KV
              label="Контраст использовался"
              value={item.contrastUsed ? "Да" : "Нет"}
            />
            <KV
              label="Оценка качества записи"
              value={item.imageQuality ?? "—"}
            />
            <KV
              label="Нужна перезапись"
