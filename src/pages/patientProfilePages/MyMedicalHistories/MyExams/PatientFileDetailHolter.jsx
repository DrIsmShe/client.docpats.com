  ...
  return (
    ...
    >
      ...
      <nav style={{ marginBottom: 12, fontSize: 14 }}>
        ...
        &nbsp;/&nbsp;
        <span>{t("details.holterShort")}</span>
      </nav>
      ...
      {!loading && !error && item && (
        <>
          ...
          {/* HOLTER-специфика */}
          <section style={card}>
            <h3 style={h3}>{t("params.holter")}</h3>
            <KV
              label={t("measure.monitorHours")}
              value={item.monitoringDuration ?? "—"}
            />
            <KV label={t("measure.maxHr")} value={item.maxHeartRate ?? "—"} />
            <KV label={t("measure.minHr")} value={item.minHeartRate ?? "—"} />
            <KV
              label={t("measure.arrhythmiaEpisodesAlt")}
            ...
            />
            <KV
              label={t("quality.recordTitle")}
              value={item.imageQuality ?? "—"}
            />
            <KV
              label={t("quality.needsRerecord")}
