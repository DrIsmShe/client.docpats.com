// client/src/pages/patient/PatientFileDetailGastroscopy.jsx
...
/* ===================== Основной компонент ===================== */
export default function PatientFileDetailGastroscopy() {
  const { id } = useParams();
  ...
  const handleDownloadPDF = async () => {
    ...
    if (imgH <= pageH - margin * 2) {
      ...
      pdf.save(`gastroscopy-summary-${item?._id || "report"}.pdf`);
      return;
    }
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      setError("");
      try {
        // 1) Текущий роут в вашем реестре с опечаткой "myy"
        const url1 = `${BASE_URL}/patient-profile/get-myy-gastroscopy-file-details/files/${encodeURIComponent(
          id
        )}`;
        let res;
        try {
          ...
          // 2) Если вы поправите префикс — пробуем корректный
          const url2 = `${BASE_URL}/patient-profile/get-my-gastroscopy-file-details/files/${encodeURIComponent(
            id
          )}`;
    ...
    >
      ...
      {!loading && !error && item && (
        <>
          ...
          {/* Специфика гастро */}
          <section style={card}>
            <h3 style={h3}>Параметры гастроскопии</h3>
            <KV
