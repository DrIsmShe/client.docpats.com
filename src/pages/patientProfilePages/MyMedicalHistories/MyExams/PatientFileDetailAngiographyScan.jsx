// client/src/pages/patient/PatientFileDetailAngiographyScan.jsx
...
/* ===================== Основной компонент ===================== */
export default function PatientFileDetailAngiographyScan() {
  ...
  const handleDownloadPDF = async () => {
    ...
    if (imgH <= pageH - margin * 2) {
      ...
      pdf.save(`angiography-summary-${item?._id || "report"}.pdf`);
      return;
    }
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      setError("");
      try {
        // Основной ожидаемый маршрут контроллера:
        const url1 = `${BASE_URL}/patient-profile/get-my-angiography-file-details/files/${encodeURIComponent(
          id
        )}`;
        let res;
        try {
          res = await axios.get(url1, { withCredentials: true });
        } catch (e1) {
          // Резерв: если бекенд назван иначе
          const url2 = `${BASE_URL}/patient-profile/get-my-angiography-details/files/${encodeURIComponent(
            id
          )}`;
          res = await axios.get(url2, { withCredentials: true });
        }
        if (!cancelled) {
  ...
  return (
    ...
    >
      ...
      <nav style={{ marginBottom: 12, fontSize: 14 }}>
        ...
        &nbsp;/&nbsp;
        <span>{t("details.angioShort")}</span>
      </nav>
      ...
      {!loading && !error && item && (
        <>
          ...
          {/* Параметры ангиографии / кардио-показатели */}
          <section style={card}>
            <h3 style={h3}>{t("params.study")}</h3>
            <KV
              label={t("measure.exposureDose")}
              value={
                typeof item.radiationDose === "number"
                  ? item.radiationDose
                  : "—"
              }
            />
            ...
            />
            <KV
              label={t("measure.dopplerFindingsEn")}
          ...
          <section style={card}>
            <h3 style={h3}>{t("links.relatedStudies")}</h3>
            <KV
              label={t("links.previousStudy")}
              value={item.previousStudy || "—"}
              mono
            />
            <KV
              label={t("links.relatedStudies")}
              value={safeJoin(
                Array.isArray(item.relatedStudies)
                  ? item.relatedStudies.map(String)
                  : []
              )}
              mono
            />
          ...
          {/* Флаги/качество/риск */}
          <section style={card}>
            <h3 style={h3}>{t("quality.andRisk")}</h3>
