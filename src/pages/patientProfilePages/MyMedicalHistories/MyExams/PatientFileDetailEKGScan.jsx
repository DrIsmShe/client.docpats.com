...
export default function PatientFileDetailEKGScan() {
  ...
  const handleDownloadPDF = async () => {
    ...
    if (imgH <= pageH - margin * 2) {
      ...
      pdf.save(`ekg-summary-${item?._id || "report"}.pdf`);
      return;
    }
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      setError("");
      try {
        // Базовый маршрут контроллера EKG:
        const url = `${BASE_URL}/patient-profile/get-my-ekg-file-details/files/${encodeURIComponent(
          id
        )}`;
