// client/src/pages/patient/PatientFileDetailEchoEKGScan.jsx
...
/* ===================== Главный компонент ===================== */
export default function PatientFileDetailEchoEKGScan() {
  const { id } = useParams();
  ...
  const handleDownloadPDF = async () => {
    ...
    if (imgH <= pageH - margin * 2) {
      ...
      pdf.save(`echoekg-summary-${item?._id || "report"}.pdf`);
      return;
    }
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      setError("");
      try {
        // Маршрут контроллера EchoEKG:
        const url = `${BASE_URL}/patient-profile/get-my-echo-ekg-file-details/files/${encodeURIComponent(
          id
        )}`;
    ...
    >
      ...
      {!loading && !error && item && (
        <>
          ...
          {/* Специфика Эхо-ЭКГ / мониторинг */}
          <section style={card}>
            <h3 style={h3}>Параметры Эхо-ЭКГ / мониторинг</h3>
            <KV
