...
// Когда сдаёмся. Ждём ДОЛЬШЕ, чем работает сам агент (у него срок 13 минут,
// RADIOLOGY_AGENT_DEADLINE_MS), — иначе клиент бросил бы опрос ровно перед
// тем, как прогон допишет отчёт, и человек решил бы, что агент завис.
// Вечный опрос при этом тоже не годится: фоновая работа живёт в памяти узла
// и рестарта не переживает, а запись осталась бы в "running" навсегда.
