    ...
    if (peer?.consent === "unsupported") {
      return (
        <div className="scribe scribe--done">
          ...
          {/* Пациент мог пересесть за компьютер и перезвонить. Сеанс,
              где его устройство писать не умеет, не запишет уже ничего:
              без этой кнопки панель показывала бы приговор, вынесенный
              прошлому звонку, и начать заново было бы нечем. Сервер на
              такой сеанс не оглядывается и заводит новый. */}
          <button
            type="button"
            className="ghost"
            disabled={busy}
          ...
          >
            {t("scribe.doctor.peerMobileRetry", "Попробовать снова")}
          </button>
        </div>
      );
    }
