...
export default function DiagnosticCasePage() {
  ...
  return (
    <div className="dg-page dg-page--narrow" dir={dir}>
      ...
      {/* ─── 4. Ответ на вопрос ────────────────────────────────────── */}
      {/* Первое, что должен прочитать врач. Раньше этого блока не было, и на
          вопрос «какой диагноз» отвечал список из восьми пунктов, где ведущая
          версия могла стоять четвёртой — потому что выводы сортируются по
          важности, а «критично/низкая уверенность» обгоняет «важно/высокая». */}
      {(data.summaries ?? []).length > 0 && (
        <section className="dg-sec dg-answer">
          <h2 className="dg-sec-title">{t("answer")}</h2>
          {data.summaries.map((s, i) => (
            <div key={i} className="dg-answer-item">
              {data.summaries.length > 1 && (
                <span className="dg-answer-mod">{s.modalityTitle}</span>
              )}
              <p className="dg-answer-text">{s.text}</p>
            </div>
          ))}
          <p className="dg-sec-note">{t("answerHint")}</p>
        </section>
      )}

      {/* ─── 5. Чего не хватает в описании ─────────────────────────── */}
