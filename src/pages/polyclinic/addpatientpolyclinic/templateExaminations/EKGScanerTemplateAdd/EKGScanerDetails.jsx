...
export default function EKGScanerDetails() {
  ...
  const downloadPDF = async () => {
    try {
      await savePdfFromElement("ekg-pdf-content", pdfBaseName());
    } catch (error) {
  ...
  const uploadPDF = async () => {
    try {
      const link = await uploadPdfFromElement("ekg-pdf-content", pdfBaseName());
      alert(`${t("EKGScanerDetails.page.messages.uploadSuccess")} ${link}`);
    } catch (error) {
