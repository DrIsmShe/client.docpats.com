...
export default function CapsuleEndoscopyScanerDetails() {
  ...
  const downloadPDF = async () => {
    try {
      await savePdfFromElement("cap-pdf-content", pdfBaseName());
    } catch (error) {
  ...
  const uploadPDF = async () => {
    try {
      const link = await uploadPdfFromElement("cap-pdf-content", pdfBaseName());
      alert(
        `${t("CapsuleEndoscopyScanerDetails.page.messages.uploadSuccess")} ${link}`,
      );
    } catch (error) {
