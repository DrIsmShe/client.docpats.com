...
export default function EchoEKGScanerDetails() {
  ...
  const downloadPDF = async () => {
    try {
      await savePdfFromElement("eek-pdf-content", pdfBaseName());
    } catch (error) {
  ...
  const uploadPDF = async () => {
    try {
      const link = await uploadPdfFromElement("eek-pdf-content", pdfBaseName());
      alert(`${t("EchoEKGScanerDetails.page.messages.uploadSuccess")} ${link}`);
    } catch (error) {
