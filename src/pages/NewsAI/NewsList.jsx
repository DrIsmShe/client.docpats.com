  ...
  useEffect(() => {
    ...
    apiInstance
      ...
      .then(({ data }) => {
        ...
        /* ФИЛЬТР БОЛЬШЕ НЕ ВКЛЮЧАЕТСЯ САМ.
         *
         * Он включался потому, что в ленте лежало восемь с половиной
         * тысяч чужих новостей, и добираться сквозь них до своих никто бы
         * не стал. Новости уехали в дайджест — в ленте остались мнения
         * врачей, их научные статьи и аналитика, и разбирать там нечего:
         * материалов пока десятки, а не тысячи.
         *
         * Зато цена ошибки перевернулась. Включённый по умолчанию фильтр
         * означает, что врач, по специальности которого ещё никто ничего
         * не написал, видит при входе ПУСТУЮ страницу — и читает её как
         * поломку платформы, а не как «по вашей теме пока пусто».
         * Проверено на живых данных: все работы на платформе написаны
         * двумя специальностями, у всех остальных лента была бы пуста.
         *
         * Кнопка осталась на месте: кому нужна своя тема — один щелчок. */
      })
          ...
          <div className="nl-content-inner">
            {loading ? (
              ...
              <div className="nl-empty">
                ...
                {/* ПУСТАЯ ЛЕНТА ДОЛЖНА ОБЪЯСНЯТЬ СЕБЯ.
                    «Моя специальность» включается врачу сама при входе, и
                    пока по его специальности нет ни одной работы, лента
                    оказывалась пустой без единого слова о причине —
                    страница читалась как поломка, хотя фильтр отработал
                    честно. Теперь причина названа, а выход из неё — в
                    одну кнопку. */}
                {onlyMine && mySpecName ? (
                  <>
                    <div className="nl-empty-title">
                      {t("feed.noneInMySpecialty", { specialty: mySpecName })}
                    </div>
                    <button
                      className="nl-empty-reset"
                      onClick={() => setOnlyMine(false)}
                    >
                      {t("feed.allTopics")}
                    </button>
