          ...
          <div className="nl-content-inner">
            {loading ? (
              ...
              <>
                ...
                >
                  ...
                  >
                    <div>
                      ...
                      >
                        Глубокий разбор медицинских новостей от AI-редактора
                        нашей платформы·
                      </div>
                    </div>
                    <div
                ...
                {heroItem && (
                  <div className="nl-hero-card-wrap">
                    <HeroCard
                      item={heroItem}
...
function HeroCard({ item, searchTerm, isAuthenticated, userRole }) {
  const meta = SOURCE_META[item._sourceType] || SOURCE_META.ai;
      ...
      <article className="nl-hero-card">
        ...
        >
          <div className="nl-hero-card-meta">
            ...
            {item.specialization && (
              <span className="nl-hero-badge nl-hero-badge-spec">
                {item.specialization}
              </span>
            )}
            {date && <span className="nl-hero-date">{date}</span>}
          </div>
          <h2 className="nl-hero-card-title">{hl(item.title)}</h2>
          {item.preview && (
            <p className="nl-hero-card-preview">{hl(item.preview)}</p>
          )}
          <div className="nl-hero-card-footer">
            {item.author?.name && (
              <span className="nl-hero-author">{hl(item.author.name)}</span>
            )}
            {item.country && (
              <>
                <span className="nl-hero-dot">·</span>
                <span className="nl-hero-country">{item.country}</span>
              </>
            )}
            <div style={{ flex: 1 }} />
            {item.likesCount > 0 && (
              <span className="nl-hero-stat">
                <svg
                  width="14"
                  height="14"
                  viewBox="0 0 24 24"
...
function DocpatsCard({ item, searchTerm, isAuthenticated, userRole }) {
      ...
      <article className="dp-card">
        <div className="dp-card-body">
          ...
          <Link
            ...
            rel="noopener noreferrer"
          >
            <h3 className="dp-card-title">{hl(item.title)}</h3>
          </Link>
          {item.preview && (
            <p className="dp-card-preview">{hl(item.preview)}</p>
          )}
          <div className="dp-card-footer">
            <div className="dp-card-author-info">
              {item.author?.name && (
                <span className="dp-author">{hl(item.author.name)}</span>
              )}
              {item.country && (
                <>
                  <span className="dp-meta-dot">·</span>
                  <span className="dp-country">{item.country}</span>
                </>
              )}
            </div>
  ...
  return (
    ...
    >
      <article className="dp-card-doctor doctor-card">
        <div className="doctor-card-inner">
          ...
          <div className=".dp-card-body-doctors doctor-card-right">
            <div className="dp-card-meta">
              ...
              {item.specialization && (
                <span className="dp-badge dp-badge-spec">
                  {item.specialization}
                </span>
              )}
            </div>
        ...
        <div className="dp-card-stats-doctor">
          <div className="dp-card-footer">
            <div className="dp-card-author-info">
              {item.country && (
                <span className="dp-country">
                  <svg
                    width="11"
                    height="11"
                    viewBox="0 0 24 24"
                    ...
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    style={{ marginRight: 3, opacity: 0.6 }}
                  >
          ...
          <div className="dp-card-stats dp-card-stats-doctor">
            {item.articlesCount > 0 && (
              <span className="dp-stat doctor-articles-stat">
                <svg
                  width="12"
                  height="12"
                  viewBox="0 0 24 24"
...
.dp-card-doctor {padding: 20px; height: 290px;background: white;border: 1px solid var(--border);border-radius: 14px;overflow: hidden;display: flex;flex-direction: column;transition: box-shadow .2s, transform .2s;box-shadow: var(--shadow-sm);}
...
.dp-card{height:300px;background:white;border:1px solid var(--border);border-radius:14px;overflow:hidden;display:flex;flex-direction:column;transition:box-shadow .2s,transform .2s;box-shadow:var(--shadow-sm)}
...
.dp-badge{font-size:10px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;padding:3px 10px;border-radius:100px;white-space:nowrap}
.dp-badge-spec{background:var(--cream2);color:var(--ink3);border:1px solid var(--border)}
.dp-card-title{font-family:var(--font-display);font-size:16px;font-weight:700;line-height:1.35;color:var(--ink);margin-bottom:8px;display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical;overflow:hidden}
.dp-card-preview{font-size:13px;color:var(--ink3);line-height:1.6;display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical;overflow:hidden;margin-bottom:14px;flex:1}
...
.dp-author{font-size:12px;font-weight:600;color:var(--ink2);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.dp-meta-dot{color:var(--border2);font-size:12px}
.dp-country{font-size:12px;color:var(--ink3);white-space:nowrap;display:flex;align-items:center;gap:3px}
.dp-card-stats{display:flex;align-items:center;gap:10px;flex-shrink:0}
.dp-stat{display:flex;align-items:center;gap:4px;font-size:12px;color:var(--ink3);font-weight:500}
...
.doctor-card-clinic{display:flex;align-items:center;gap:5px;font-size:12px;color:var(--ink3);margin-bottom:6px;font-style:italic;line-height:1.4}
