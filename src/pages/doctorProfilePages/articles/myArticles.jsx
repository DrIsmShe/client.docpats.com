...
export default function MyArticles() {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      ...
      setError(null);
      try {
        const resp = await axios.get(`${API_BASE}/doctor-profile/my-articles`, {
          withCredentials: true,
          headers: {
            "Accept-Language": i18n.language, // 🔥 ВАЖНО
          },
    ...
    fetchArticles();
  }, [page, perPage, i18n.language, refresh]);
  useEffect(() => {
    if (!articles.length) return;

    const hasOriginals = articles.some((a) => a.isOriginal);

    if (!hasOriginals) return;

    const interval = setInterval(() => {
      console.log("🔄 refreshing translations...");
      setRefresh((r) => r + 1); // ✅ теперь работает
    }, 4000);
